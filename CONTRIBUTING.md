...
1. Fork https://github.com/vlang/onecontext using GitHub's interface to your own account.
   Let's say that the forked repository is at
   `https://github.com/YOUR_GITHUB_USERNAME/v` .
2. Clone the main onecontext repository https://github.com/vlang/onecontext to a local folder on
   your computer, say named onecontext/ (`git clone https://github.com/vlang/onecontext onecontext`)
3. `cd onecontext`
...
7. On GitHub's web interface, go to: https://github.com/vlang/onecontext/pulls
...
9. If there are merge conflicts, or a branch lags too much behind onecontext's master,
...
The point of doing the above steps, is to never directly push to the main onecontext
repository, _only to your own fork_. Since your local `master` branch tracks the
main onecontext repository's master, then `git checkout master`, as well as
...
1. `hub clone vlang/onecontext my_onecontext`
2. `cd my_onecontext`
...
### Improve onecontext by making commits:
...
(so that your changes can be merged to the main onecontext repository)
